...
class EmailConfirmationViewModel @Inject constructor(
    ...
    private val _emailConfirmationActionStateFlow = MutableStateFlow<SubmissionCodeActionState>(
        SubmissionCodeActionState.Loading
    )

    private val _codeSendingActionStateFlow = MutableStateFlow<SendingCodeActionState>(
        SendingCodeActionState.Loading
    )
                ...
                when (val response = authInteractor.verifyRegistrationCode(email, code)) {
                    ...
                    is NetworkResponse.ServerError -> {
                        _emailConfirmationActionStateFlow.emit(
                            SubmissionCodeActionState.VerificationServerError(
                                response
                            )
                        )
                    }
                    is NetworkResponse.NetworkError -> {
                        _emailConfirmationActionStateFlow.emit(
                            SubmissionCodeActionState.NetworkError(
                                response
                            )
                        )
                    }
                    is NetworkResponse.UnknownError -> {
                        _emailConfirmationActionStateFlow.emit(
                            SubmissionCodeActionState.UnknownError(
                                response
                            )
                        )
                    }
    ...
    private suspend fun createProfile(
        ...
        try {
            when (val response = authInteractor.createProfile(
                ...
                is NetworkResponse.ServerError -> {
                    _emailConfirmationActionStateFlow.emit(
                        SubmissionCodeActionState.CreateProfileServerError(
                            response
                        )
                    )
                }
        ...
        viewModelScope.launch {
            ...
            try {
                when (val response = authInteractor.sendRegistrationVerificationCode(email)) {
                    ...
                    is NetworkResponse.ServerError -> {
                        _codeSendingActionStateFlow.emit(
                            SendingCodeActionState.SendingServerError(
                                response
                            )
                        )
                    }
                    is NetworkResponse.NetworkError -> {
                        _codeSendingActionStateFlow.emit(
                            SendingCodeActionState.NetworkError(
                                response
                            )
                        )
                    }
                    is NetworkResponse.UnknownError -> {
                        _codeSendingActionStateFlow.emit(
                            SendingCodeActionState.UnknownError(
                                response
                            )
                        )
                    }
